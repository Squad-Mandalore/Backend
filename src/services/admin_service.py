...
from src.schemas.admin_schema import AdminPostSchema
...
def create_admin(admin_post_schema: AdminPostSchema, db: Session) -> Administrator:
    user_db: User | None = db.scalar(
        select(User).where(User.username == admin_post_schema.username)
    )
...
def update_admin(
    id: str, admin_patch_schema: AdminPatchSchema, db: Session
) -> Administrator:
    admin: Administrator = get_admin_by_id(id, db)
    if (
        admin_patch_schema.username is not None
        and admin.username != admin_patch_schema.username
    ):
        user_db: User | None = db.scalar(
            select(User).where(User.username == admin_patch_schema.username)
        )
        if user_db is not None:
            raise HTTPException(
    ...
    if admin_patch_schema.unhashed_password is not None:
    ...
    admin.last_edited_at = datetime.now()
    db.commit()
    return cast(Administrator, admin)
