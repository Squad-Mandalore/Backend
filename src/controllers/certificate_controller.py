...
from src.schemas.certificate_schema import CertificateResponseSchema, CertificatePostSchema
...
@router.get("/{id}", response_model=list[CertificateResponseSchema], status_code=status.HTTP_200_OK)
async def get_certificates(db: Session = Depends(get_db)) -> Certificate:
    return certificate_service.get_certificates_by_id(id, db)

@router.delete("/{id}", response_model=list[CertificateResponseSchema], status_code=status.HTTP_200_OK)
async def delete_certificate(db: Session = Depends(get_db)) -> None:
    return certificate_service.delete_certificate(id, db)

@router.post("/", response_model=list[CertificateResponseSchema], status_code=status.HTTP_200_OK)
async def create_certificate(certificate_post_schema: CertificatePostSchema, db: Session = Depends(get_db)) -> Certificate:
...
async def update_certificate(id: str, certificate_patch_schema: CertificateResponseSchema, db: Session = Depends(get_db)) -> Certificate:
