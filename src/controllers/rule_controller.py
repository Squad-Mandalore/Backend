...
@router.get(
    '/', response_model=list[RuleResponseSchema], status_code=status.HTTP_200_OK
)
...
@router.post(
    '/', response_model=RuleResponseSchema, status_code=status.HTTP_201_CREATED
)
async def create_rule(
    rule_post_schema: RulePostSchema,
...
@router.patch(
    '/{id}', response_model=RuleResponseSchema, status_code=status.HTTP_202_ACCEPTED
)
async def update_rule(
    id: str,
    rule_patch_schema: RulePatchSchema,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> Rule:
