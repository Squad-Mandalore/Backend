...
@router.get("/", response_model=list[AdminResponseSchema], status_code=status.HTTP_200_OK)
async def get_all_admins(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[Administrator]:
    return admin_service.get_all_admins(db)

@router.get("/{id}", response_model=AdminResponseSchema, status_code=status.HTTP_200_OK)
async def get_admin(id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Administrator:
...
async def update_admin(id: str, admin_patch_schema: AdminPatchSchema, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Administrator:
