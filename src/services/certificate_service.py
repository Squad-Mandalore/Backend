...
def create_certificate(
    athlete_id, title, blob: UploadFile, user_id: str, db: Session
) -> Certificate:
    blob_data = blob.file.read()
    certificate_post_schema = CertificatePostSchema(
        athlete_id=athlete_id, title=title, blob=blob_data
    )
...
def update_certificate(
    id: str, certificate_patch_schema: CertificatePatchSchema, db: Session
) -> Certificate:
