...
from src.schemas.exercise_schema import (
    ExercisePatchSchema,
    ExercisePostSchema,
...
# exercise routes
@router.get(
    '/', response_model=list[ExerciseResponseSchema], status_code=status.HTTP_200_OK
)
...
@router.get(
    '/{id}', response_model=ExerciseResponseSchema, status_code=status.HTTP_200_OK
)
...
@router.post(
    '/', response_model=ExerciseResponseSchema, status_code=status.HTTP_201_CREATED
)
async def create_exercise(
    exercise_post_schema: ExercisePostSchema,
...
@router.patch(
    '/{id}', response_model=ExerciseResponseSchema, status_code=status.HTTP_202_ACCEPTED
)
async def update_exercise(
    id: str,
    exercise_patch_schema: ExercisePatchSchema,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> Exercise:
