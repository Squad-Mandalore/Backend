...
from src.schemas.completes_schema import (
    CompletesPatchSchema,
    CompletesPostSchema,
...
# completes routes
@router.get(
    '/', response_model=list[CompletesResponseSchema], status_code=status.HTTP_200_OK
)
async def get_completes(
    ...
    return completes_service.get_completes_by_id(
        exercise_id, athlete_id, tracked_at, db
    )
...
@router.post(
    '/', response_model=CompletesResponseSchema, status_code=status.HTTP_201_CREATED
)
async def create_completes(
    completes_post_schema: CompletesPostSchema,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> Completes:
...
@router.patch(
    '/', response_model=CompletesResponseSchema, status_code=status.HTTP_202_ACCEPTED
)
async def update_completes(
    ...
    return completes_service.update_completes(
        exercise_id, athlete_id, tracked_at, completes_patch_schema, user.id, db
    )
