...
@router.post(
    '/refresh', response_model=auth_schema.Token, status_code=status.HTTP_200_OK
)
async def refresh_access_token(
    x_refresh_token: str = Header(), db: Session = Depends(get_db)
):
...
@router.get(
    '/whoami',
    response_model=user_schema.UserResponseSchema,
