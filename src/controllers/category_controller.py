...
from src.schemas.category_schema import CategoryExerciseFullResponseSchema, CategoryFullResponseSchema, CategoryResponseSchema, CategoryVeryFullResponseSchema
...
@router.get("/{id}", response_model=CategoryFullResponseSchema, status_code=status.HTTP_200_OK)
async def get_category_by_id(id: str, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> Category:
