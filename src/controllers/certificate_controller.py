...
@router.get("/{id}", response_model=CertificateSingleResponseSchema, status_code=status.HTTP_200_OK)
async def get_certificates(id: str, db: Session = Depends(get_db)) -> CertificateSingleResponseSchema:
    certificate = certificate_service.get_certificates_by_id(id, db)
    certificate_data = CertificateSingleResponseSchema(
        blob=base64.b64encode(certificate.blob).decode('utf-8'),
...
async def create_certificate(blob: UploadFile = File(...), athlete_id: str = Form(...), title: str = Form(...),
