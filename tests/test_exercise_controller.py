...
def test_create_exercise(session: Session, client: TestClient) -> None:
    ...
    session.commit()
    body = {'title': 'title', 'category_id': category.id}
    response: Response = client.post(
        '/exercises', json=body, headers=TestVariables.headers
    )
...
def test_patch_exercise(client: TestClient) -> None:
    ...
    response: Response = client.patch(
        f'/exercises/{exercise_id}', json=body, headers=TestVariables.headers
    )
