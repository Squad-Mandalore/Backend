...
def create_athlete(
    athlete_post_schema: AthletePostSchema, trainer_id: str, db: Session
) -> Athlete:
    user_db: User | None = db.scalar(
        select(User).where(User.username == athlete_post_schema.username)
    )
...
def update_athlete(
    id: str, athlete_patch_schema: AthletePatchSchema, db: Session
) -> Athlete:
    athlete: Athlete = get_athlete_by_id(id, db)
    if (
        athlete_patch_schema.username is not None
        and athlete.username != athlete_patch_schema.username
    ):
        user_db: User | None = db.scalar(
            select(User).where(User.username == athlete_patch_schema.username)
        )
        if user_db is not None:
            raise HTTPException(
    ...
    if athlete_patch_schema.unhashed_password is not None:
    ...
    athlete.last_edited_at = datetime.now()
    db.commit()
    return cast(Athlete, athlete)
